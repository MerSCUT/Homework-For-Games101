...
{
    // angle : degree of rotation angle.
    // axis : a vector indicate the rotation axis.
    ...
    N << 0, -z, y, 
        z, 0, -x, 
        -y, x, 0;
    model = std::cos(alpha) * Matrix3f::Identity() + (1.0f - std::cos(alpha)) * axis * axis.transpose()
        + std::sin(alpha) * N ;
    Matrix4f ret = Matrix4f::Zero();
    ret.topLeftCorner(3, 3) = model;
    ret(3, 3) = 1.0f;
    return ret;
}
...
int main(int argc, const char** argv)
{
    ...
    rst::rasterizer r(700, 700);    // It's a rasterizer
